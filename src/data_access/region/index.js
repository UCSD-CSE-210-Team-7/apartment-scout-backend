...
async function getUsers(region) {
  const query =
    "SELECT email from USERRegion JOIN Users USING (email) WHERE IS_SCOUT=TRUE AND ZIPCODE=$1";
  const args = [region];
...
async function getRegions(user) {
  const query =
    "SELECT zipcode from UserRegion JOIN Users USING (email) WHERE IS_SCOUT=TRUE AND email=$1";
  const args = [user];
  const res = await client.query(query, args);
  return res.rows;
}
